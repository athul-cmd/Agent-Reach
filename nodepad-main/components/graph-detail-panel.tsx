...
export function GraphDetailPanel({
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-l border-border/60 bg-card">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="px-4 pt-4 pb-3">
          {editingText ? (
            ...
            <p
              className={`text-base font-bold leading-relaxed text-foreground rounded-sm px-2 py-1 -mx-2 transition-colors ${readOnly ? "" : "cursor-text hover:bg-secondary/20"} ${block.isEnriching ? "shimmer-text" : ""}`}
              onDoubleClick={() => {
                if (readOnly) return
        ...
        {(block.annotation || editingAnnotation) && (
          <div className="px-4 pb-3 space-y-1.5">
            {editingAnnotation ? (
              ...
              <div
                className={`text-sm leading-relaxed text-muted-foreground rounded-sm px-2 py-1 -mx-2 transition-colors border-l-2 pl-3 ${readOnly ? "" : "cursor-text hover:bg-secondary/20"}`}
      ...
      {!readOnly && isTypePickerOpen && pickerRect && createPortal(
        <div
