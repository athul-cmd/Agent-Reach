...
export const TileCard = memo(function TileCard({ 
  ...
  const handleDoubleClick = useCallback((e: React.MouseEvent) => {
    ...
    setEditText(block.text)
    setIsEditing(true)
  }, [block.text, block.annotation, effectiveCollapsed, readOnly])
    ...
    >
      ...
      >
        ...
        <div className={`flex items-center gap-2 flex-shrink-0 ${isTextRTL ? 'flex-row-reverse' : ''}`} style={{ color: "inherit" }}>
          ...
          {!readOnly && !effectiveCollapsed && block.contentType === "thesis" && (
            <button
              onClick={(e) => {
                e.stopPropagation()
          ...
          {!readOnly && !effectiveCollapsed && onTogglePin && (
            <button
              onClick={(e) => {
                e.stopPropagation()
          ...
          {!readOnly && onChangeType && !effectiveCollapsed && (
            <button
      ...
      {!readOnly && isTypePickerOpen && pickerRect && onChangeType && isMounted && createPortal(
        <div
